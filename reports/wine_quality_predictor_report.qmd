...
3. What are the optimal hyperparameters for our Random Forest model?
...
report_path = Path("..") / "results" / "evaluation" / "classification_report.txt"
...
def parse_report(text: str) -> pd.DataFrame:
    ...
    for i, ln in enumerate(lines):
        if ln.startswith("precision") and "recall" in ln and "f1-score" in ln and "support" in ln:
            header_idx = i
            break

    if header_idx is None:
        raise ValueError("Classification report header not found.")
    ...
    base = [c for c in df["Class"].tolist() if c not in {"accuracy", "macro avg", "weighted avg"}]
    order = base + ["accuracy", "macro avg", "weighted avg"]
...
if not report_path.exists():
    raise FileNotFoundError(f"Classification report not found at: {report_path.resolve()}")
